...
class PenyakitController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->nama != $penyakit->nama) {
            $rules['nama'] = 'unique:penyakit|max:500|required'; // PERBAIKAN: unique:penyakit bukan unique:users
            $messages['nama.unique'] = 'Penyakit sudah ada!'; // PERBAIKAN: pesan error untuk penyakit
            $messages['nama.required'] = 'Penyakit wajib diisi!'; // PERBAIKAN: pesan error untuk penyakit
            $messages['nama.max'] = 'Penyakit tidak boleh lebih dari :max karakter!'; // PERBAIKAN: pesan error untuk penyakit
        }
        ...
        // Filter hanya field yang diizinkan untuk update
        $data = [
            'kode' => $request->kode,
            'nama' => $request->nama,
            'bobot' => $request->bobot
